          ...
          <AnimatePresence>
            {menuOpen && (
              <Dialog.Portal forceMount>
                {/* Dimming + frosted scrim (material, pushes background back) */}
                ...
                {/* Panel grows from the trigger (top-right), spring settle */}
                <Dialog.Content asChild forceMount aria-describedby={undefined}>
                  <motion.div
                    ...
                    className="fixed top-4 right-4 z-50 w-64 rounded-2xl border border-line bg-white/75 backdrop-blur-xl p-5 shadow-[0_24px_60px_-20px_rgba(147,51,234,0.4)] focus:outline-none"
                  >
                    ...
                    <nav className="flex flex-col gap-1">
                      {navLinks.map((link) => (
                        <a
                          ...
                          className="py-2 text-base font-medium text-ink-soft hover:text-ink transition-colors"
                        >
                          {link.label}
                        </a>
                      ))}
