...
// --- GTA6 neon palette (violet -> magenta -> rose/red) ---
const NEON: Array<[number, number, number]> = [
  [124, 58, 237], // #7c3aed violet
  [147, 51, 234], // #9333ea purple
  [192, 38, 211], // #c026d3 magenta
  [219, 39, 119], // #db2777 pink
  [225, 29, 72], // #e11d48 rose/red
];

// Interpolate a color along the neon gradient (t in 0..1) -> "rgb(r,g,b)"
function neonColor(t: number): string {
  t = Math.max(0, Math.min(1, t));
  const seg = t * (NEON.length - 1);
  const i = Math.floor(seg);
  const f = seg - i;
  const a = NEON[i];
  const b = NEON[Math.min(i + 1, NEON.length - 1)];
...
class AnimationController {
  ...
  private readonly startDotYOffset = 10;
  ...
  private drawStartDot() {
    if (this.time > this.changeEventTime) {
      ...
      // bright neon core with glow
      this.ctx.save();
      this.ctx.fillStyle = "#ffd1ec";
  ...
  public render() {
    ...
    // spiral trail (glowing neon core)
    this.drawTrail(t1);

    // stars (each carries its own neon color)
    ctx.shadowBlur = 0;
  ...
  private drawTrail(t1: number) {
    ...
    for (let i = 0; i < this.trailLength; i++) {
      ...
      const color = neonColor(i / this.trailLength);
...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    // neon color biased by spiral position for a cohesive gradient field
    this.color = neonColor(this.spiralLocation * 0.7 + Math.random() * 0.3);
  }
...
export function SpiralAnimation() {
  ...
  // SSR-safe: start at 0 and measure on mount
  ...
  useEffect(() => {
    ...
    // Draw into a SQUARE buffer and display it as a centered square that
    // covers the viewport, so the spiral stays a true circle (never squished
    // into an oval by a wide/tall viewport).
